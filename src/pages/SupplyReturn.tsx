...
const SupplyReturn = () => {
  ...
  const handlePrintBS = (order: SupplyOrder) => {
    try {
      ...
      drawSummaryRow('Montant HT:', `${order.subtotal.toFixed(2)} DH`, summaryY + 10);
      drawSummaryRow(`TVA (${order.taxRate}%):`, `${order.taxAmount.toFixed(2)} DH`, summaryY + 18);
      drawSummaryRow('Total TTC:', `${order.total.toFixed(2)} DH`, summaryY + 28, primaryColor, true);
  ...
  const calculatePaymentDebt = () => {
    const { subtotal, taxAmount } = calculatePaymentTotals();
    ...
    // Driver debt only includes gas sales, excluding consigne (deposits)
    return Math.max(0, (subtotal + taxAmount) - (cash + check + mygaz));
  };
  ...
  const handlePaymentSubmit = () => {
    ...
    toast({
      ...
      description: `Paiement de ${(cash + check + mygaz).toFixed(2)} DH enregistré avec succès${debt > 0 ? `. Dette de ${debt.toFixed(2)} DH ajoutée au chauffeur.` : '.'}`,
    });
  };
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-slate-50/80 hover:bg-slate-50/80">
                      ...
                      <TableHead className="text-center font-semibold text-slate-700">Vides (Sortie)</TableHead>
                        ...
                        >
                          ...
                          <TableCell className="text-center">
                            <Input
                              ...
                              className="w-20 mx-auto text-center border-slate-200 focus:ring-indigo-500"
                            />
                          </TableCell>
                          <TableCell className="text-center">
            ...
            <CardContent className="space-y-6">
              <div className="space-y-4">
                <div className="flex justify-between items-center pb-2 border-b border-white/10">
                  ...
                  <span className="text-lg font-medium">{subtotal.toFixed(2)} DH</span>
                </div>
                <div className="flex justify-between items-center pb-2 border-b border-white/10">
                  <span className="text-indigo-200">TVA (10%)</span>
                  <span className="text-lg font-medium">{taxAmount.toFixed(2)} DH</span>
                </div>
                <div className="flex justify-between items-center pt-2">
                  <span className="text-xl font-bold">Total TTC</span>
                  <span className="text-3xl font-black text-emerald-400">{total.toFixed(2)} DH</span>
                </div>
              </div>
                        ...
                        filteredOrders.map((order, idx) => (
                          <TableRow key={order.id} className="hover:bg-slate-50/50 transition-colors">
                            ...
                            <TableCell className="text-right">
                              <div className="font-black text-slate-900">{order.total.toFixed(2)} DH</div>
                            </TableCell>
                            <TableCell>
                      ...
                      <div className="mt-6 p-4 bg-muted/30 rounded-lg border">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          <div className="p-2 bg-background rounded border">
                            ...
                            <p className="font-bold text-green-600">{(paymentInfo.cash || 0).toFixed(2)} DH</p>
                          </div>
                          <div className="p-2 bg-background rounded border">
                            <p className="text-xs text-muted-foreground">Chèque</p>
                            <p className="font-bold text-blue-600">{(paymentInfo.check || 0).toFixed(2)} DH</p>
                          </div>
                          <div className="p-2 bg-background rounded border">
                            <p className="text-xs text-muted-foreground">MYGAZ</p>
                            <p className="font-bold text-blue-800">{(paymentInfo.mygaz || 0).toFixed(2)} DH</p>
                          </div>
                          <div className="p-2 bg-background rounded border">
                            <p className="text-xs text-muted-foreground">Dette Restante</p>
                            <p className="font-bold text-red-600">{(paymentInfo.debt || 0).toFixed(2)} DH</p>
                          </div>
                        </div>
                        <div className="mt-3 pt-3 border-t flex justify-between items-center">
                          <span className="text-sm font-medium">Montant Total du Bon:</span>
                          <span className="text-lg font-bold">{(paymentInfo.total || 0).toFixed(2)} DH</span>
                        </div>
                      </div>
                    );
                  }
                    ...
                    {(paymentCashAmount || paymentCheckAmount || paymentMygazAmount) && (
                      <div className="bg-orange-50 border border-orange-200 p-4 rounded-lg">
                        ...
                        <div className="text-sm text-orange-700">
                          Calcul: ({(calculatePaymentTotals().subtotal + calculatePaymentTotals().taxAmount).toFixed(2)}) - ({(parseFloat(paymentCashAmount) || 0).toFixed(2)} + {(parseFloat(paymentCheckAmount) || 0).toFixed(2)} + {(parseFloat(paymentMygazAmount) || 0).toFixed(2)}) = {calculatePaymentDebt().toFixed(2)} DH
                        </div>
                        <p className="text-[10px] text-orange-600 mt-1 italic">* Hors Consigne et R.C. comme demandé</p>
