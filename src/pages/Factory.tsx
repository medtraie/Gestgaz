...
const Factory = () => {
  ...
  const handleDownloadInvoicePDF = (invoice: Invoice) => {
    try {
      ...
      const bottleTypeSummary: Record<string, { name: string, sent: number, received: number }> = {};
        ...
        if (op) {
          op.sentBottles.forEach(b => {
            if (!bottleTypeSummary[b.bottleTypeId]) {
              ...
              bottleTypeSummary[b.bottleTypeId] = { name: bt?.name || 'Inconnu', sent: 0, received: 0 };
            }
      ...
      if (Object.keys(bottleTypeSummary).length > 0) {
        ...
        autoTable(doc, {
          startY: finalY + 20,
          head: [['Type de Bouteille', 'Total Envoyé', 'Total Reçu', 'Différence']],
  ...
  const handleDownloadPDF = (operation: FactoryOperation) => {
    try {
      ...
      doc.setFontSize(10);
      doc.setFont('helvetica', 'normal');
      doc.text(`Réf: #${operation.id}`, 210 - 14, 32, { align: 'right' });
      ...
      drawInfoLabel('Camion / Matricule', truck ? `${truck.name} (${truck.plateNumber})` : 'N/A', 112, 85);
      ...
      autoTable(doc, {
        startY: 120,
        head: [['Direction', 'Type de Bouteille', 'État', 'Quantité']],
        body: tableData,
        theme: 'grid',
        headStyles: {
          fillColor: primaryColor,
      ...
      if (finalY < 230) {
        ...
        if (operation.debtChange !== 0) {
          ...
          drawSummaryRow(label, value, summaryY + 32, color, true);
        }
  ...
  const exportOperationsPDF = () => {
    try {
      ...
      autoTable(doc, {
        startY: 75,
        head: [['Date', 'Chauffeur', 'Envoyé', 'Reçu', 'Statut', 'Dette/Diff.']],
  ...
  const [invoices, setInvoices] = useState<Invoice[]>(() => {
    const saved = localStorage.getItem('factoryInvoices');
    return saved ? JSON.parse(saved) : [];
  });
  ...
  // Sauvegarder dans le stockage local lors du changement de données
  React.useEffect(() => {
  ...
  React.useEffect(() => {
    localStorage.setItem('factoryInvoices', JSON.stringify(invoices));
  }, [invoices]);
  ...
  const handleDebtSettlement = () => {
    ...
    if (settlementForm.type === 'empty') {
      ...
      updateEmptyBottlesStockByBottleType(settlementForm.bottleTypeId, settlementForm.quantity);
    } else {
  ...
  const handleReturnFromFactory = () => {
    ...
    receivedBottles.forEach(bottle => {
      ...
      if (currentBT) {
        ...
        updateEmptyBottlesStockByBottleType(bottle.bottleTypeId, bottle.quantity);
      }
    });
  ...
  const handleUpdateInvoice = () => {
    ...
    setInvoices(prev => prev.map(inv => inv.id === originalInvoiceId ? editingInvoice : inv));
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
        {[
          ...
          { label: 'Dette Fournisseur (V)', value: suppliers.reduce((s, sup) => s + (sup.debts?.reduce((acc, d) => acc + d.emptyDebt, 0) || 0), 0), icon: Package, color: 'text-orange-600', bg: 'bg-orange-50' },
          { label: 'Défectueux en attente', value: suppliers.reduce((s, sup) => s + (sup.debts?.reduce((acc, d) => acc + d.defectiveDebt, 0) || 0), 0), icon: AlertTriangle, color: 'text-rose-600', bg: 'bg-rose-50' }
        ].map((stat, idx) => (
          <motion.div 
            key={idx} 
                            ...
                            <div className="flex-1 space-y-2">
                              ...
                              <Input
                                ...
                                onChange={(e) => {
                                  ...
                                  if (bottleIndex !== -1) {
                                    newBottles[bottleIndex].emptyQuantity = value;
                            ...
                            <div className="flex-1 space-y-2">
                              ...
                              <Input
                                ...
                                onChange={(e) => {
                                  ...
                                  const newBottles = [...sendForm.bottles];
                                  if (bottleIndex !== -1) {
                                    newBottles[bottleIndex].defectiveQuantity = value;
                                    setSendForm({...sendForm, bottles: newBottles});
                                  }
                                }}
        ...
        <DialogContent className="max-w-2xl p-0 overflow-hidden border-none rounded-3xl shadow-2xl">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="text-left">
                  ...
                  <DialogDescription className="text-indigo-100 mt-1">
                    Sélectionnez les bons de livraison à regروper dans cette facture
                  </DialogDescription>
                </div>
              </div>
            </DialogHeader>
          ...
          <div className="p-8 space-y-6 bg-white max-h-[60vh] overflow-y-auto">
            <div className="space-y-4">
              ...
              <Input 
                value={suppliers.find(s => s.id === selectedSupplierForInvoice)?.name || ''} 
                disabled 
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  {bottleTypes.map((bt, index) => (
                    <div key={bt.id} className="bg-slate-50 p-6 rounded-2xl border border-slate-100 flex items-center justify-between">
                      ...
                      <div className="w-32 text-left space-y-2">
                                      ...
                                      <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-4">
                                          ...
                                          <div className="text-left">
                                            ...
                                            <p className="text-slate-400 text-sm mt-1">{operation.driverName} • {new Date(operation.date).toLocaleDateString('fr-FR')}</p>
                                          </div>
                                        </div>
                    ...
                    <AnimatePresence mode="popLayout">
                      {debtSettlements.length > 0 ? (
                        debtSettlements.map((settlement, idx) => {
                          const supplier = suppliers.find(s => s.id === settlement.supplierId);
